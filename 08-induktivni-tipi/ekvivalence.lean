...
∀ {xs acc : List A},
 (reverse'' xs) ++ acc = reverse'.aux xs acc :=
  by
    intro xs --ce velja za vse potem velja za poljubnega, torej si predstavimo poljubnega in ,u dajmo ime xs
    induction xs with
    | nil => 
...
theorem reverse''_eq_reverse' {A : Trpe} : ∀ {xs : List A}, reverse'' xs = reverse' xs :=
  by
    intro xs
    induction xs with
    | nil => 
